...
| S2 | Retrieval improves from v1-ab baseline; CRD/deployment/ownership categories improve | **Incomplete — bounded pilot completed; full-corpus evaluation required** | Reproducible baseline: `benchmark/consumer-v1/results/v1-ab/scored-results.json` (40 questions, tree_a overall avg 0.3625, tree_b 0.3375). Four experiment conditions available (manifest v1.3.0, validator PASS). Context telemetry wired. A 32-session provisional pilot (4/40 questions, $8.1087, 0 failures, 347.65 s) completed with exact-match scoring; results are directional only and not statistically significant — see `docs/tasks/done/run-authorized-provisional-32-session-pilot.md` and artifacts under `tmp/provisional-pilot/`. The 84%/94-question figure is **unverified external historical feedback** — no artifact exists | Requires full-corpus paid evaluation with user authorization; bounded pilot provides directional signal only (4/40 questions, exact-match) |
...
| User authorization | **Bounded pilot authorized and completed; full-corpus authorization not obtained** | A 32-session provisional pilot was authorized and completed ($8.1087, 4/40 questions, 0 failures, 347.65 s; see `docs/tasks/done/run-authorized-provisional-32-session-pilot.md`). No full-corpus evaluation may run without separate explicit authorization stating expected cost and duration | Explicit user authorization for full-corpus evaluation |
...
The legacy route is preserved. The plan explicitly states: "Retire legacy only after a canary shows no regression in analyzer-fact accuracy, improves retrieval on the question corpus, and does not degrade human review outcomes." A bounded 32-session provisional pilot completed (4/40 questions, $8.1087, 0 failures, 347.65 s, local MLflow read-back verified), but it is not a full canary — it covered 4 of 40 questions with exact-match scoring only, no human review, no semantic calibration, and no external MLflow registration. The legacy route must remain available until the full rollout gates are satisfied.
