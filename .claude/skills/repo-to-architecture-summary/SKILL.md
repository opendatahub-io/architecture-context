...
**IMPORTANT FOR OPERATORS** (legacy and partial routes only — synthesis agents skip this):
Don't just read manifests - read the controller code!
...
> **Synthesis route**: The output file was pre-seeded by the orchestrator. Use **Edit** operations to refine it — do NOT rewrite the entire file with Write. Skip to Step 7b after editing.
>
> **Partial route**: Use Edit for incremental changes or Write if gap-filling requires a full regeneration.
...
> **Synthesis and partial routes**: Skip this step. The orchestrator validates the merged output after the agent completes. Do NOT attempt to run the validation script — it requires Bash, which is denied by the execution guard for these routes.

**Legacy route only**: After writing, run the validation script to catch template conformance errors:
