...
def test_metadata_only_candidate_delta_is_not_substantive(tmp_path: Path):
    body = architecture_document("Service", "Analyzer purpose.") + "\n" + (
        "Detailed analyzer text.\n" * 80
    )
    preseed = tmp_path / "preseed.md"
    ...
    candidate.write_text(
        body.replace(
            "- **Generated By**: Test Model",
            "- **Generated By**: Claude Opus 4.6 on 2026-07-28",
        )
    )

    has_delta, reason = architecture._candidate_has_substantive_delta(
        candidate, preseed
    )

    assert has_delta is False
    assert "no substantive candidate changes" in reason


def test_recovery_rejects_unchanged_preseed_candidate(tmp_path: Path):
    body = architecture_document("Service", "Analyzer purpose.") + "\n" + (
        "Detailed analyzer text.\n" * 80
    )
    generation_dir = tmp_path / ".generation"
    generation_dir.mkdir()
    preseed = generation_dir / "preseed.md"
    candidate = generation_dir / "candidate.md"
    preseed.write_text(body)
    candidate.write_text(body)
    result = {"name": "example", "success": False, "error": ""}
    job = {
        "name": "example",
        "candidate_path": candidate,
        "preseed_path": preseed,
    }

    recovered = architecture._recover_agent_result(job, result, tmp_path / "logs")

    assert recovered["success"] is False
    assert recovered["error"] == (
        "agent produced no substantive candidate changes; candidate matches preseed"
    )
