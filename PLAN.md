...
- [Add Targeted Pipeline Subcommand](docs/tasks/done/add-targeted-pipeline-subcommand.md) — 2026-07-30 (`pipeline` subcommand runs selected phases over repeated component/repo selectors; `custom-test.sh` captures the four-component runtime replay)
...
- [Reconcile Kueue CRD Count Scope](docs/tasks/done/reconcile-kueue-crd-count-scope.md) — 2026-07-30 (`FACT-007` explicit 11 core API CRDs / 16 total CRD/API rows contract; focused re-eval passed 1.0/1.0)
- [Finish Consumer V1 Scoring and Scope Cleanup](docs/tasks/done/finish-consumer-v1-scoring-scope-cleanup.md) — 2026-07-30 (`primary_overall` architecture-scope scoring, primary-only flagged regressions, non-primary diagnostics, and current exact-match variant cleanup)
- [Rework Consumer V1 Rolling Inventory Questions](docs/tasks/done/rework-consumer-v1-rolling-inventory-questions.md) — 2026-07-30 (`NAV-008` now checks flat `rhoai.next` document layout and accepts `PLATFORM.md` as a secondary citation source; unchanged `20260730T110242Z` raw results rescore with no flagged regressions)
...
- [Add Consumer V1 rhoai.next Evaluation Wrapper](docs/tasks/done/add-consumer-v1-rhoai-next-eval-wrapper.md) — 2026-07-28 (one command validates inputs, runs consumer-v1 A/B evaluation from `tmp/architecture-context/architecture/rhoai.next` to `architecture/rhoai.next`, scores results, and writes report artifacts under `tmp/evaluations/`)
- [Fix Consumer Eval Auth and Sidecar Boundary](docs/tasks/done/fix-consumer-eval-auth-and-sidecar-boundary.md) — 2026-07-28 (consumer-v1 evaluator now fails auth/login responses, loads Claude env without shell-sourcing `.env`, and evaluates sanitized trees without `.analyzer` / `.generation` sidecars)
...
- [Partial Route Gap Replay](docs/notes/partial-route-gap-replay-2026-07-30.md)
