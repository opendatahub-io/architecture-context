...
|---------|----------------|----|-------|
| Prometheus | monitoring | metrics-target | Metrics scraping via service annotations at /q/metrics |
...
- **Prometheus:** Inbound scrape; role: metrics-target; protocol: HTTP; purpose: Metrics collection via prometheus.io/scrape annotation at /q/metrics. [source: explainability-service/manifests/base/trustyai-deployment.yaml:9-11]
- **KServe/ModelMesh:** Inbound payload consumer; role: data-source; protocol: HTTP; purpose: Receives model inference input/output payloads via POST /consumer/kserve/v2. The Deployment init container creates a model-serving-config ConfigMap that configures ModelMesh to forward payloads to TrustyAI. [source: explainability-service/src/main/java/org/kie/trustyai/service/endpoints/consumer/ConsumerEndpoint.java:29-59, explainability-service/manifests/base/trustyai-deployment.yaml:62-81]
- **MariaDB (optional):** Outbound JDBC; role: persistence; protocol: JDBC; purpose: Optional database storage via Hibernate ORM when QUARKUS_HIBERNATE_ORM_ACTIVE=true. [source: explainability-service/src/main/resources/application.properties:63-72]

| Component | Interaction Type | Role | Port | Protocol | Encryption | Purpose |
|---------|----------------|----|----|--------|----------|-------|
| Prometheus | Inbound scrape | metrics-target | 8080 | HTTP | Unknown | Metrics collection via prometheus.io/scrape annotation at /q/metrics |
