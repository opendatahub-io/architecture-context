...
- **http_endpoints (observed)**: 1 http_endpoints facts extracted [source: ci/cached-builds/make_test.py:174]
- **services (observed)**: 1 services facts extracted [source: ci/cached-builds/make_test.py:174]
...
- **Question:** Where is authentication enforced for this surface, and is it conditional?
  ...
  **Candidate:** `ci/cached-builds/make_test.py`:174 (HTTP API, None (no auth middleware detected))
  **Status:** candidate; **Limitations:** candidate location only; source inspection is required to establish the relationship
### configuration_lifecycle

- **Question:** What lifecycle, command, probes, and deployment configuration surround this entrypoint?
  **Expected signal:** main command, startup path, probe, signal handling, or workload mapping
  **Candidate:** `codeserver/ubi9-python-3.12/Dockerfile.konflux.cpu`:363 (codeserver/ubi9-python-3.12/Dockerfile.konflux.cpu:CMD)
  **Status:** candidate; **Limitations:** candidate location only; source inspection is required to establish the relationship
- **Question:** What lifecycle, command, probes, and deployment configuration surround this entrypoint?
  **Expected signal:** main command, startup path, probe, signal handling, or workload mapping
...
- **Question:** What lifecycle, command, probes, and deployment configuration surround this entrypoint?
  **Expected signal:** main command, startup path, probe, signal handling, or workload mapping
  **Candidate:** `jupyter/tensorflow/ubi9-python-3.12/Dockerfile.konflux.cuda`:101 (jupyter/tensorflow/ubi9-python-3.12/Dockerfile.konflux.cuda:ENTRYPOINT)
  **Status:** candidate; **Limitations:** candidate location only; source inspection is required to establish the relationship
- **Question:** What lifecycle, command, probes, and deployment configuration surround this entrypoint?
  **Expected signal:** main command, startup path, probe, signal handling, or workload mapping
  **Candidate:** `jupyter/trustyai/ubi9-python-3.12/Dockerfile.konflux.cpu`:95 (jupyter/trustyai/ubi9-python-3.12/Dockerfile.konflux.cpu:ENTRYPOINT)
  **Status:** candidate; **Limitations:** candidate location only; source inspection is required to establish the relationship
### http_endpoints

- **Question:** Does this endpoint have additional dynamic routes or a concrete handler/owner?
  **Expected signal:** route registration, handler binding, middleware, or owner symbol
  **Candidate:** `ci/cached-builds/make_test.py`:174 (PATCH, time.sleep)
  **Status:** candidate; **Limitations:** candidate location only; source inspection is required to establish the relationship
### services

- **Question:** Which workload owns this Service and does its target port match a runtime listener?
  **Expected signal:** selector, target deployment, port mapping, or listener
  **Candidate:** `ci/cached-builds/make_test.py`:174 (notebooks)
  **Status:** candidate; **Limitations:** candidate location only; source inspection is required to establish the relationship
...
- HTTP API methods=All mechanism=None (no auth middleware detected) enforcement=FastAPI/Starlette application policy=No authentication middleware registered [source: ci/cached-builds/make_test.py:174]
### http_endpoints

- PATCH time.sleep on port ; transport= encryption=Configurable auth=Unknown owner= [source: ci/cached-builds/make_test.py:174]

## Cross-Cutting Evidence

### deployment_topology

- **observed**: Service notebooks targets  with 0 port(s) [source: ci/cached-builds/make_test.py:174]
...
- **observed**: All HTTP API uses None (no auth middleware detected) at FastAPI/Starlette application; policy=No authentication middleware registered [source: ci/cached-builds/make_test.py:174]
- **dependency-signal**: auth-middleware targets pyjwt: JWT/OAuth authentication library dependency [source: codeserver/ubi9-python-3.12/requirements.cpu.txt:468]
- **dependency-signal**: rbac-ref targets kubernetes: Kubernetes client library (RBAC capable) [source: codeserver/ubi9-python-3.12/requirements.cpu.txt:270]
