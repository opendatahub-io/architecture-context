...
|----|------|----|--------|---------|----------|----|-----|-------|
| /healthz | GET | 8081 | HTTP | HTTP/1.1 | None | None | cmd | Health probe (healthz.Ping) |
| /readyz | GET | 8081 | HTTP | HTTP/1.1 | None | None | cmd | Readiness probe; gates on ConfigMap bootstrap completion |
...
|------------|----|----|-----------|--------|----------|----|--------|
| controller-manager-metrics-service | ClusterIP | 8443/TCP | 8443 | TCP | TLS (self-signed by default) | TokenReview + SAR | Internal |
...
| Prometheus | Configurable | HTTP or HTTPS | TLS 1.2+ (configurable) | Bearer token (optional) | Metrics queries for autoscaling decisions |
